...
def cell_param_stat(stream_file):
    # get cell parameters from input stream file 
    ...
    df_statistics = pd.DataFrame(index=['mean', 'std'],  columns = ['a', 'b', 'c', 'alpha', 'beta', 'gamma'])
...
print("BEFORE PROCEEDING: each reserv_* directory has its own det_shift_cell_stat.py, because there is a unique name of stream file on each directory!")
print("Example of stream file name format: ---- exp_19638_run_r0038-0040_type_d+l_delay_10e-9_dist_%d_reserv_7_.stream----")
print("GO BACK TO PREVIOUS STEPS IF YOU DONT HAVE THAT STREAM FILE")
...
stream_file_name = "exp_19638_run_r0038-0040_type_d+l_delay_10e-9_dist_%d_reserv_7_.stream" % (closest_detdist)
