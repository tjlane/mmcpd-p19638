...
PGROUP=/sf/alvra/data/p19638/work/DetDist_opti/reserv_DEFAULT #Please modify the path #
SAMPLEFOLDER=/sf/alvra/data/p19638/ # this is the folder in which the data are stored
SAVEFOLDER=/sf/alvra/data/p19638/work/DetDist_opti/reserv_DEFAULT
#EXPERIMENT=$(sed 's/_/-/g' <<< "$SAMPLEFOLDER")
CELL=/sf/alvra/data/p19638/work/files-process/pl_mmCPD_euXFEL_vAlex.cell # best specify absolute path #
GEOMETRY=/sf/alvra/data/p19638/work/vapostolop/test_DetDist/24-06-2022.geom # best specify absolute path, remember to use a recent mask file inside the geometry file #
...
RESERV=DEFAULT # $2 # 
RUN=0022 # $3 has to have 4 digits e.g. 0001 or 0123 or 1467 #
TYPE=DEFAULT_TYPE # $4 has to be dark or light
DELAY=DEFAULT_DELAY # $5 # data collection FOLDERS have to be named in that way e.g. 10e-12 for 10 ps, 1e-6 for 1us #
...
indexamajig -j \$NPROC -i ${PGROUP}/dark_reserv_$RESERV.lst  -g ${GEOMETRY}-${DIST}.geom -o ${SAVEFOLDER}/run_${RUN}_type_${TYPE}_delay_${DELAY}_dist_${DIST}_reserv_$RESERV_DetDist.stream --indexing=$INDEXING --peaks=peakfinder8 --threshold=$THRESHOLD -p $CELL --int-radius=$INTRADIUS --min-snr=$SNR --min-peaks=$MINPEAKS --min-pix-count=$MINPIXCOUNT > ${SAVEFOLDER}/run_${RUN}_type_${TYPE}_delay_${DELAY}_dist_${DIST}_reserv_${RESERV}DetDist.log 2>&1
EOF
done
